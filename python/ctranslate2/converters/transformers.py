...
class TransformersConverter(Converter):
    ...
    def _load(self):
        with torch.no_grad():
            ...
            if model_class is not None and hasattr(loader, "get_model_class"):
            ...
            if model_class is None:
                if self._model_name_or_path.startswith("efficient-speech/lite-whisper"):
                    model_class = transformers.AutoModel
                else:
                    raise ValueError(
                        "The model %s is not supported by the converter."
